...
public partial class Program
{
	...
	static void ChanceForRandomBattle()
	{
		...
		{
			Battle(Map == Maps.Castle ? Enemy.Guard : Enemy.Boar, out _);
			if (!gameRunning)
			{
				return;
			}
	...
	{
		Battle(Enemy.GuardBoss, out bool ranAway);
		if (!gameRunning)
		{
			return;
		}
	...
	{
		Battle(Enemy.FinalBoss, out bool ranAway);
		if (!gameRunning)
		{
			return;
		}
	...
	static void Battle(Enemy Enemy, out bool ranAway)
	{
		movesSinceLastBattle = 0;
		ranAway = false;

		int enemyHealth = Enemy switch
		{
			Enemy.Boar      => 01,
			Enemy.GuardBoss => 10,
			Enemy.Guard     => 05,
		...
		string[] animationRight = Enemy switch
			{
				Enemy.Boar => Sprites.IdleBoar,
				Enemy.Guard => Sprites.IdleLeft,
				Enemy.GuardBoss => Sprites.IdleLeft,
				Enemy.FinalBoss => Sprites.IdleLeft,
				_ => [Sprites.Error],
			};
			...
			while (Console.KeyAvailable)
			{
				...
				switch (key)
				{
					...
					case ConsoleKey.D2 or ConsoleKey.NumPad2:
						if (!pendingConfirmation)
						{
							bool success = Enemy switch
							{
								Enemy.Boar => Random.Shared.Next(10) < 9,
								Enemy.Guard => Random.Shared.Next(10) < 7,
								_ => true,
							};
							if (success)
							{
					...
					case ConsoleKey.Enter:
						if (pendingConfirmation)
						{
							...
							if (enemyHealth <= 0)
							{
								if (Enemy is Enemy.FinalBoss)
								{
									return;
								}
								int experienceGain = Enemy switch
								{
									Enemy.Boar => 1,
									Enemy.GuardBoss => 20,
									Enemy.Guard => 10,
									Enemy.FinalBoss => 9001, // ITS OVER 9000!
									_ => 0,
								};
								Console.Clear();
								Console.WriteLine();
...
if (cj * 8 + ci >= 0 && cj * 8 + ci < characterMapRender.Length)
{
    sb.Append(characterMapRender[cj * 8 + ci]);
}
else
{
    // Tangani kasus ketika indeks di luar batas
    sb.Append(" "); // Misalnya, append spasi sebagai cadangan
}
					continue;
				}
